...
Y = picardsFormula(func, x_initial, y_initial, increment, initial_range-increment);
for i = 1 : length(Y)
    F(i) = round(func(X(i), Y(i)), 4);
end
P = Y; C = Y;
for i = length(Y) : length(X)-1
    ...
    function [y, f] = milnesPredictor(n, f, X, Y, F, h)
        y = round(Y(n-3) + ((4*h)/3)*(2*F(n-2)-F(n-1)+2*F(n)), 4);
        f = round(f(X(n+1), y), 4);
    end
    function [y, f] = milnesCorrector(n, f, X, Y, F, h)
        y = round(Y(n-1) + (h/3)*(F(n-1)+4*F(n)+F(n+1)), 4);
        f = round(f(X(n+1), y), 4);
    end
end
% f = @(x, y) (x^2 + y^2) / 10;
